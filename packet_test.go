...
func TestEncodeDecodePacketHeader(t *testing.T) {
	...
	f := func(seq, ack uint16, acks uint32) bool {
		header := PacketHeader{seq: seq, ack: ack, acks: acks}
...
func BenchmarkMarshalPacketHeader(b *testing.B) {
	header := PacketHeader{seq: math.MaxUint16, ack: math.MaxUint16, acks: math.MaxUint32}
...
func BenchmarkUnmarshalPacketHeader(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if recovered.seq != header.seq || recovered.ack != header.ack || recovered.acks != header.acks {
